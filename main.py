...
async def guess(ctx):
    ...
    for i in range(tris):
        try:
            ...
            return await ctx.send(lang(ctx.guild.id)["phrases"]["guess_fail_time"].format(num))
        guess = int(msg.content)
        if guess == num:
            await ctx.send(lang(ctx.guild.id)["phrases"]["guess_win"].format(num))
            return
        else:
            if guess > num:
                await ctx.send(lang(ctx.guild.id)["phrases"]["guess_smaller"])
            else:
                await ctx.send(lang(ctx.guild.id)["phrases"]["guess_bigger"])
    await ctx.send(lang(ctx.guild.id)["phrases"]["guess_fail_tries"].format(num))
